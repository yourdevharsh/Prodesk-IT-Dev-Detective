...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const userAvatarEl = document.querySelector(".profileCard1 img");
  const userNameEl = document.querySelector(".profileCard1 .userName");
  const userBioEl = document.querySelector(".profileCard1 .userBio");
  const userJoinDateEl = document.querySelector(".profileCard1 .userJoinDate");
  const userPortfolioEl = document.querySelector(
    ".profileCard1 .userPortfolio",
  );
  const userLatestReposEl = document.querySelector(
    ".profileCard1 .userLatestRepos",
  );
  ...
  function renderProfile1(
    ...
    for (const repo of latestRepos) {
      ...
      repoP.innerHTML = `<a href="${url}"> ${name} </a> <span> ${formatDate(date)} </span>`;
